...
class GameCanvasElement():
    ...
    # Subclass can use this to customize their behavior.
    def init_canvas_object(self):
        """A method that a subclass can overide to initialize itself AFTER
...
class GameApp(ttk.Frame):
    ...
    # refactor - add parameters instead of acccessing attributes
    def create_canvas(self, width, height):
        canvas = tk.Canvas(self, borderwidth=0, width=width, height=height,
                           highlightthickness=0)
        # replace strings with named constant.
    ...
    def remove_element(self, element: GameCanvasElement):
        ...
        if element in self.elements:
            ...
            # also remove it from canvas
            self.canvas.delete(element.canvas_object_id)

    # Hook method or Callback
    def init_game(self):
        pass
